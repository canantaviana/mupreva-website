...
class page
{
    ...
    public function get_menu_data($parent)
    {
        ...
        $options->sql_filter     = "`parent` = '{$parent_search}'"; // such ["mupreva2564_1"]
        $options->order         = 'norder ASC';
    ...
    public function get_menu_tree_plain($term_id = WEB_MENU_PARENT, $exclude = [])
    {
        ...
        if (empty($exclude)) {
            ...
            foreach ($data->result as $key => $value) {
                $current_section_id = (int)$value->section_id;
                if (!in_array($current_section_id, $exclude)) {
                    $ar_data[] = $value;
                }
            }
        }
...
<ul class="main-nav link-dn" id="main-nav">
    ...
    public function resolve_column_value($column_obj, $row = false)
    {
        ...
        if (property_exists($row, $column_obj->colname)) {
            ...
            # Post-process some complex elements
    ...
    public function get_portal_value($column_obj, $row, $max_records = 5, $offset = 0)
    {
        ...
        # Configure object with additional info always
        ...
        $portal_template_map = $column_obj;
        $temp_page_mode      = 'list';
        ...
        $i = 1;
        foreach ($rows_data->result as $portal_row) {
    ...
    public function get_childrens_value($column_obj, $row, $max_records = 100, $offset = 0)
    {
        ...
        $childrens_template_map = $column_obj;
        $temp_page_mode          = 'list';
        $table_name             = $this->area_table; // Same table like current page
        ...
        foreach ($ar_value as $key => $current_locator) {
            ...
            $i++;
        } //end foreach ($ar_value as $key => $section_id)
    ...
    /**
     * GET_MENU_LINK
     * Build normal link, but besides, check if link element exists in menu (children case) and in this case
    ...
    public function set_template_from_table($area_table, $area_name, $custom = null)
    {
        ...
        if (SHOW_DEBUG === true) {
            error_log("!!! Mapped table '$area_table' to template_map name: '$template_name'");
        }
