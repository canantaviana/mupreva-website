...
<form action="#" class="search-form">
    ...
    */
    search_rows: function (options) {
        ...
        const mapfilter = "((geolocalizacion is not null and geolocalizacion != '') or (geolocalizacion_produccion is not null and geolocalizacion_produccion != ''))";
        let sql_filter = parsed_filter
            ? '(' + parsed_filter + ') and ('+mapfilter+')'
            : mapfilter
    ...
    */
    render_data: function (options) {
        ...
        return new Promise(function (resolve) {
            ...
            const list_data = page.parse_map_data(ar_rows) // prepares data to use in list
            self.list = self.list || new map_factory() // creates / get existing instance of list
            self.list
            .init({
