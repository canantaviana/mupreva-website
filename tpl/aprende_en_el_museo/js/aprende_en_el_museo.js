...
var biblio = {
    ...
    set_config: function (options) {
        ...
        if (options) {
            ...
            localStorage.setItem(
                "aprende_config",
                JSON.stringify(self.biblio_config)
            );
        }
...
<form action="#" class="search-form">
    ...
    form_submit: function (form_obj, options = {}) {
        ...
        const scroll_result =
            typeof options.scroll_result === "boolean"
                ? options.scroll_result
                : true;
    ...
    search_rows: function (options) {
        ...
        const offset = options.offset || self.pagination.offset;
        const count =
            typeof options.count !== "undefined" ? options.count : true;
    ...
    author: function (item) {
        ...
        const ar_authors_name =
            authors_name && authors_name.length > 0
                ? authors_name.split(" | ")
                : [""];

        const ar_authors_surname =
            authors_surname && authors_surname.length > 0
